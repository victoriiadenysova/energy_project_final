var R=Object.defineProperty;var H=(t,e,s)=>e in t?R(t,e,{enumerable:!0,configurable:!0,writable:!0,value:s}):t[e]=s;var L=(t,e,s)=>(H(t,typeof e!="symbol"?e+"":e,s),s);import{i as p}from"./vendor-f93ed741.js";function O(){document.querySelector(".filter-list-js").classList.add("hidden-form");const e=document.querySelector(".wrap-js"),s=document.createElement("p");s.className="favorite-text",e.appendChild(s),s.textContent="It appears that you haven`t added any exercises to your favorites yet. To get started, you can add exercises that you like to your favorites for easier access in the future."}const c="/energy_project/assets/sprite-c2a9f1af.svg",h=(t,e)=>t.length<=e?t:`${t.slice(0,e)}...`;function N(t){const e=document.querySelector(".filter-list-js");e.classList.remove("hidden-form"),t.length>6?e.classList.add("scroll"):e.classList.remove("scroll");const s=t.map(({_id:o,target:i,name:n,bodyPart:a,burnedCalories:r,time:$})=>`
  <li class="favourites_list_item" id=${o}>
  ...
  </li>`).join("");e.innerHTML=s}function b(){listOfFavorites();const t=JSON.parse(localStorage.getItem("favorites"));if(!(t!=null&&t.length)){O();return}N(t)}const q=document.getElementById("home"),B=document.getElementById("favorites"),C=document.getElementById("home-text"),k=document.getElementById("favorites-text");document.getElementById("navigation");const A=window.location.href.toString();let F=A.slice(-14);const U=()=>{F==="favorites.html"?(q.classList.remove("active"),C.classList.remove("black"),B.classList.add("active"),k.classList.add("black"),b()):(q.classList.add("active"),C.classList.add("black"),B.classList.remove("active"),k.classList.remove("black"))};U();document.getElementById("burger-button");const J=document.getElementById("burger");document.getElementById("burger-button-close");const D=t=>{t.key==="Escape"&&J.classList.add("visually-hidden"),document.body.classList.remove("disable-scroll")};document.addEventListener("keydown",D);class P{constructor(){this.overlay=document.querySelector(".overlay"),this.modal=null,this.closeButton=null,this.closeButtonHandler=()=>this.close(),this.escapeKeyHandler=e=>this.closeEsc(e),this.overlayClickHandler=e=>this.closeBack(e)}open(e){var s,o;this.overlay||(this.overlay=document.querySelector(".overlay")),this.overlay&&(this.overlay.innerHTML=e,this.overlay.style.zIndex=4,this.overlay.style.display="flex",this.modal=this.overlay.querySelector(".modal-info")||this.overlay.querySelector(".modal-get-raiting"),(s=this.modal)==null||s.classList.remove("visually-hidden"),this.closeButton=this.overlay.querySelector(".modal-button-close"),document.body.classList.add("no-scroll"),(o=this.closeButton)==null||o.addEventListener("click",this.closeButtonHandler),document.addEventListener("keydown",this.escapeKeyHandler),this.overlay.addEventListener("click",this.overlayClickHandler))}close(){var e,s;this.overlay&&((e=this.modal)==null||e.classList.add("visually-hidden"),this.overlay.innerHTML="",this.overlay.style.display="none",this.overlay.style.zIndex=-1,document.body.classList.remove("no-scroll"),(s=this.closeButton)==null||s.removeEventListener("click",this.closeButtonHandler),document.removeEventListener("keydown",this.escapeKeyHandler),this.overlay.removeEventListener("click",this.overlayClickHandler),this.modal=null,this.closeButton=null)}closeEsc(e){e.key==="Escape"&&this.close()}closeBack(e){e.target===this.overlay&&this.close()}}const _=new P,l=class{static handleErrors(e){return async()=>{try{return await e()}catch(s){console.error("Помилка при запиті: ",s.message)}}}static async fetchJson(e,s){const o=await fetch(e,s);if(!o.ok){const n=await o.text().catch(()=>"");throw new Error(`HTTP ${o.status} ${o.statusText} | ${e} | ${n.slice(0,200)}`)}return(o.headers.get("content-type")||"").includes("application/json")?o.json():o.text()}async getQuotes(){return await l.handleErrors(async()=>await l.fetchJson(`${l.BASE_URL}/quote`))()}async getByFilterName(e){return await l.handleErrors(async()=>{const o=new URLSearchParams({...e});return await l.fetchJson(`${l.BASE_URL}/filters?${o}`)})()}async getByFilterCategory(e){return await l.handleErrors(async()=>{const o=new URLSearchParams({...e});return await l.fetchJson(`${l.BASE_URL}/exercises?${o}`)})()}async getOneExercises(e){return await l.handleErrors(async()=>await l.fetchJson(`${l.BASE_URL}/exercises/${e}`))()}async addExercisesRate(e,s){return await l.handleErrors(async()=>{const i={method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)};return await l.fetchJson(`${l.BASE_URL}/exercises/${e}/rating`,i)})()}async addSubscription(e){return await l.handleErrors(async()=>{const o={method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(e)};return await l.fetchJson(`${l.BASE_URL}/subscription`,o)})()}};let g=l;L(g,"BASE_URL","https://your-energy.b.goit.study/api");const E=new g,m=class{open(){(!m.loaderEl||m.loaderEl.classList.contains("js-loader"))&&(m.loaderEl.style.display="block")}close(){m.loaderEl&&(m.loaderEl.style.display="none")}};let y=m;L(y,"loaderEl",document.querySelector(".js-loader"));const d=new y;function z(){return{quoteText:document.querySelector(".quote-text"),quoteAuthor:document.querySelector(".quote-author")}}async function j(){const{quoteText:t,quoteAuthor:e}=z();if(!t||!e)return;const s=localStorage.getItem("quote"),o=new Date().toDateString();if(s)try{const i=JSON.parse(s);if((i==null?void 0:i.date)===o&&(i!=null&&i.quote)){t.textContent=i.quote??"",e.textContent=i.author??"";return}}catch{}await K(t,e)}async function K(t,e){try{d.open();const s=await E.getQuotes();if(!s)return;const{author:o,quote:i}=s;localStorage.setItem("quote",JSON.stringify({author:o??"",quote:i??"",date:new Date().toDateString()})),t.textContent=i??"",e.textContent=o??""}finally{d.close()}}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",j,{once:!0}):j();function Q(){const t=document.createElement("button");t.className="scroll-up-button hidden";const e=document.createElementNS("http://www.w3.org/2000/svg","svg");e.setAttribute("viewBox","0 0 32 32"),e.setAttribute("class","button-icon");const s=document.createElementNS("http://www.w3.org/2000/svg","use");s.setAttributeNS("http://www.w3.org/1999/xlink","href",`${c}#icon-arrow`),e.appendChild(s),t.appendChild(e),t.addEventListener("click",function(){window.scrollTo({top:0,behavior:"smooth"})}),document.addEventListener("scroll",function(){window.scrollY>50?t.classList.remove("hidden"):t.classList.add("hidden")}),document.body.appendChild(t)}Q();function G(t){if(t)return t[0].toUpperCase()+t.slice(1)}function I(t,e=0){const s=Number(t);return Number.isFinite(s)?s:e}function u(t,e="—"){if(t==null)return e;const s=String(t).trim();return s||e}function X(t){const e=I(t,0),s=Math.floor(e),o=e-s,i=Math.round(o*100);let n='<div class="rating-container-not-cursore" data-rating="0">';for(let r=1;r<=s;r+=1)n+=`<span class="star-js selected" data-value="${r}">
    ...
    </span>`;return n+"</div>"}function Y(t){return t?`<button class="add-favorite-js" type="button" style="font-size: 14px;">
  ...
  </button>`}function Z(t){const e=I(t==null?void 0:t.rating,0),s=Number.isFinite(e)?e.toFixed(1):"—";return`<div class="modal-info" data-id="${u(t==null?void 0:t._id,"")}">
    ...
    <div class="modal-content-wrapper">
      <div class="card-wrapper">
        <h3 class="title-card-modal">${G(u(t==null?void 0:t.name,""))}</h3>
  ...
  </div>`}function V(t){return`<div class="modal-get-raiting" data-id="${u(t,"")}">
    <button class="modal-button-close" id="button-close">
      <svg class="close-icon-img" width="20" height="20" aria-label="close-icon">
        <use href="${c}#icon-close"></use>
      </svg>
  ...
  </div>`}function W(){const t=document.querySelector(".rating-container-js"),e=t.querySelectorAll(".exercises_btn_start_icon_rating");let s=t.querySelector(".user-rating-js");e.forEach((a,r)=>{a.addEventListener("mouseover",()=>o(r)),a.addEventListener("mouseout",i),a.addEventListener("click",()=>n(r+1))});function o(a){i();for(let r=0;r<=a;r++)e[r].classList.add("hovered"),s.textContent=`${r+1}`}function i(){e.forEach(a=>{const r=t.dataset.rating;a.classList.remove("hovered"),s.textContent=`${r||0}`})}function n(a){t.setAttribute("data-rating",a),e.forEach((r,$)=>{$+1<=a?r.classList.add("selected"):r.classList.remove("selected")})}}let f=[];const v=JSON.parse(localStorage.getItem("favorites"));v!=null&&v.length&&f.push(...v);function tt(t){updateFirestore(t),f.push(t),localStorage.setItem("favorites",JSON.stringify(f))}const M=t=>{deleteFavorites(t),f=f.filter(e=>e._id!==t),localStorage.setItem("favorites",JSON.stringify([...f]))};p.settings({timeout:3e3,resetOnHover:!0,icon:"material-icons",transitionIn:"flipInX",transitionOut:"flipOutX",position:"topRight"});class et{error(e){p.error({title:"Error",message:e})}success(e){p.success({title:"OK",message:e})}info(e){p.info({title:"Info",message:e})}}const w=new et,st=document.querySelector(".filter-list-js");let S,x,T;async function ot(t){t.preventDefault();const e=document.querySelector(".modal-get-raiting").dataset.id,o=document.querySelector(".rating-container-js").dataset.rating,i=document.querySelector(".raiting-form-field-input").value,n=document.querySelector(".raiting-form-field-comment").value,a={rate:Number(o),email:i,review:n};d.open();const r=await E.addExercisesRate(e,a);d.close(),r.message?w.error(`${r.message}`):(w.success(`Thank you for your mark - ${a.rate} for ${r.name}`),_.close())}async function it(){const t=document.querySelector(".modal-info").dataset.id;_.close(),_.open(V(t)),W(),T=document.querySelector(".raiting-form"),T.addEventListener("submit",ot)}async function rt(t){const e=document.querySelector(".refresh-button-js"),s=document.querySelector(".modal-info").dataset.id;let i=window.location.href.toString().slice(-14);if(e.dataset.favorite==="false"){e.innerHTML=`<button class="add-favorite-js" type="button">
                                        <span class="remote-favorites">Remove from favorites</span>
                                        <svg class="trash-icon-img" width="18" height="18" aria-label="trash-icon">
                                            <use href="${c}#icon-trash"></use>
                                        </svg>
                                    </button>`,e.dataset.favorite="true",d.open();let n=await E.getOneExercises(s);d.close(),tt(n),i==="favorites.html"&&b()}else e.innerHTML=`<button class="add-favorite-js" type="button">
                                        <span>Add to favorites</span>
                                        <svg class="heart-icon-img" width="20" height="20" aria-label="heart-icon">
                                            <use href="${c}#icon-heart"></use>
                                        </svg>
                                    </button>`,e.dataset.favorite="false",M(s),i==="favorites.html"&&b()}function nt(t){var i,n,a;const e=t.target;if(e.classList.contains("favourites_btn_trash_icon")){const r=e.dataset.id||((n=(i=e.closest("[data-id]"))==null?void 0:i.dataset)==null?void 0:n.id);if(!r)return;M(r),b();return}const s=e.closest("[data-id]"),o=(a=s==null?void 0:s.dataset)==null?void 0:a.id;o&&(e.classList.contains("favourites_btn_workout")||at(o))}st.addEventListener("click",nt);const at=async t=>{if(!t)return;let e;try{d.open(),e=await E.getOneExercises(t)}catch(o){w.error((o==null?void 0:o.message)||String(o));return}finally{d.close()}if(!e){w.error("Exercise not found or request failed");return}let s=!1;try{const o=localStorage.getItem("favorites"),i=o?JSON.parse(o):[];s=Array.isArray(i)&&i.some(n=>(n==null?void 0:n._id)===t)}catch{s=!1}e.favorite=s,_.open(Z(e)),x=document.querySelector(".refresh-button-js"),S=document.querySelector(".add-rating"),S&&S.addEventListener("click",it),x&&x.addEventListener("click",rt)};export{h as a,G as c,E as f,d as l,w as m,c as s};
//# sourceMappingURL=exercises-right-part-filter-97af0d73.js.map
