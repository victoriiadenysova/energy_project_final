var R=Object.defineProperty;var H=(t,e,s)=>e in t?R(t,e,{enumerable:!0,configurable:!0,writable:!0,value:s}):t[e]=s;var E=(t,e,s)=>(H(t,typeof e!="symbol"?e+"":e,s),s);import{s as c}from"./applyAssets-85d2affc.js";import{i as v}from"./vendor-f93ed741.js";function O(){document.querySelector(".filter-list-js").classList.add("hidden-form");const e=document.querySelector(".wrap-js"),s=document.createElement("p");s.className="favorite-text",e.appendChild(s),s.textContent="It appears that you haven`t added any exercises to your favorites yet. To get started, you can add exercises that you like to your favorites for easier access in the future."}const h=(t,e)=>t.length<=e?t:`${t.slice(0,e)}...`;function N(t){const e=document.querySelector(".filter-list-js");e.classList.remove("hidden-form"),t.length>6?e.classList.add("scroll"):e.classList.remove("scroll");const s=t.map(({_id:o,target:i,name:r,bodyPart:a,burnedCalories:n,time:L})=>`
  <li class="favourites_list_item" id=${o}>
  ...
  </li>`).join("");e.innerHTML=s}let f=JSON.parse(localStorage.getItem("favorites"))||[];function A(t){const e=f.findIndex(s=>s._id===t._id);e>=0?f[e]=t:f.push(t),localStorage.setItem("favorites",JSON.stringify(f))}const M=t=>{f=f.filter(e=>e._id!==t),localStorage.setItem("favorites",JSON.stringify(f))};function y(){const t=JSON.parse(localStorage.getItem("favorites"));if(!(t!=null&&t.length)){O();return}N(t)}const q=document.getElementById("home"),B=document.getElementById("favorites"),k=document.getElementById("home-text"),C=document.getElementById("favorites-text");document.getElementById("navigation");const U=window.location.href.toString();let F=U.slice(-14);const J=()=>{F==="favorites.html"?(q.classList.remove("active"),k.classList.remove("black"),B.classList.add("active"),C.classList.add("black"),y()):(q.classList.add("active"),k.classList.add("black"),B.classList.remove("active"),C.classList.remove("black"))};J();const x=()=>{const t=document.getElementById("burger");t&&(t.classList.remove("is-open"),document.body.classList.remove("disable-scroll"),setTimeout(()=>{t.classList.contains("is-open")||t.classList.add("visually-hidden")},300))};document.addEventListener("click",t=>{if(t.target.closest("#burger-menu-open")){const i=document.getElementById("burger");i&&(i.classList.remove("visually-hidden"),setTimeout(()=>{i.classList.add("is-open")},10),document.body.classList.add("disable-scroll"));return}if(t.target.closest("#burger-button-close")){x();return}t.target.closest(".burger-pages-list-item")&&x()});document.addEventListener("keydown",t=>{t.key==="Escape"&&x()});class D{constructor(){this.overlay=document.querySelector(".overlay"),this.modal=null,this.closeButton=null,this.closeButtonHandler=()=>this.close(),this.escapeKeyHandler=e=>this.closeEsc(e),this.overlayClickHandler=e=>this.closeBack(e)}open(e){var s,o;this.overlay||(this.overlay=document.querySelector(".overlay")),this.overlay&&(this.overlay.innerHTML=e,this.overlay.style.zIndex=4,this.overlay.style.display="flex",this.modal=this.overlay.querySelector(".modal-info")||this.overlay.querySelector(".modal-get-raiting"),(s=this.modal)==null||s.classList.remove("visually-hidden"),this.closeButton=this.overlay.querySelector(".modal-button-close"),document.body.classList.add("no-scroll"),(o=this.closeButton)==null||o.addEventListener("click",this.closeButtonHandler),document.addEventListener("keydown",this.escapeKeyHandler),this.overlay.addEventListener("click",this.overlayClickHandler))}close(){var e,s;this.overlay&&((e=this.modal)==null||e.classList.add("visually-hidden"),this.overlay.innerHTML="",this.overlay.style.display="none",this.overlay.style.zIndex=-1,document.body.classList.remove("no-scroll"),(s=this.closeButton)==null||s.removeEventListener("click",this.closeButtonHandler),document.removeEventListener("keydown",this.escapeKeyHandler),this.overlay.removeEventListener("click",this.overlayClickHandler),this.modal=null,this.closeButton=null)}closeEsc(e){e.key==="Escape"&&this.close()}closeBack(e){e.target===this.overlay&&this.close()}}const b=new D,l=class{static handleErrors(e){return async()=>{try{return await e()}catch(s){console.error("Помилка при запиті: ",s.message)}}}static async fetchJson(e,s){const o=await fetch(e,s);if(!o.ok){const r=await o.text().catch(()=>"");throw new Error(`HTTP ${o.status} ${o.statusText} | ${e} | ${r.slice(0,200)}`)}return(o.headers.get("content-type")||"").includes("application/json")?o.json():o.text()}async getQuotes(){return await l.handleErrors(async()=>await l.fetchJson(`${l.BASE_URL}/quote`))()}async getByFilterName(e){return await l.handleErrors(async()=>{const o=new URLSearchParams({...e});return await l.fetchJson(`${l.BASE_URL}/filters?${o}`)})()}async getByFilterCategory(e){return await l.handleErrors(async()=>{const o=new URLSearchParams({...e});return await l.fetchJson(`${l.BASE_URL}/exercises?${o}`)})()}async getOneExercises(e){return await l.handleErrors(async()=>await l.fetchJson(`${l.BASE_URL}/exercises/${e}`))()}async addExercisesRate(e,s){return await l.handleErrors(async()=>{const i={method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)};return await l.fetchJson(`${l.BASE_URL}/exercises/${e}/rating`,i)})()}async addSubscription(e){return await l.handleErrors(async()=>{const o={method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(e)};return await l.fetchJson(`${l.BASE_URL}/subscription`,o)})()}};let p=l;E(p,"BASE_URL","https://your-energy.b.goit.study/api");const w=new p,m=class{open(){(!m.loaderEl||m.loaderEl.classList.contains("js-loader"))&&(m.loaderEl.style.display="block")}close(){m.loaderEl&&(m.loaderEl.style.display="none")}};let g=m;E(g,"loaderEl",document.querySelector(".js-loader"));const d=new g;function P(){return{quoteText:document.querySelector(".quote-text"),quoteAuthor:document.querySelector(".quote-author")}}async function j(){const{quoteText:t,quoteAuthor:e}=P();if(!t||!e)return;const s=localStorage.getItem("quote"),o=new Date().toDateString();if(s)try{const i=JSON.parse(s);if((i==null?void 0:i.date)===o&&(i!=null&&i.quote)){t.textContent=i.quote??"",e.textContent=i.author??"";return}}catch{}await z(t,e)}async function z(t,e){try{d.open();const s=await w.getQuotes();if(!s)return;const{author:o,quote:i}=s;localStorage.setItem("quote",JSON.stringify({author:o??"",quote:i??"",date:new Date().toDateString()})),t.textContent=i??"",e.textContent=o??""}finally{d.close()}}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",j,{once:!0}):j();function K(){const t=document.createElement("button");t.className="scroll-up-button hidden";const e=document.createElementNS("http://www.w3.org/2000/svg","svg");e.setAttribute("viewBox","0 0 32 32"),e.setAttribute("class","button-icon");const s=document.createElementNS("http://www.w3.org/2000/svg","use");s.setAttributeNS("http://www.w3.org/1999/xlink","href",`${c}#icon-arrow`),e.appendChild(s),t.appendChild(e),t.addEventListener("click",function(){window.scrollTo({top:0,behavior:"smooth"})}),document.addEventListener("scroll",function(){window.scrollY>50?t.classList.remove("hidden"):t.classList.add("hidden")}),document.body.appendChild(t)}K();function Q(t){if(t)return t[0].toUpperCase()+t.slice(1)}function I(t,e=0){const s=Number(t);return Number.isFinite(s)?s:e}function u(t,e="—"){if(t==null)return e;const s=String(t).trim();return s||e}function G(t){const e=I(t,0),s=Math.floor(e),o=e-s,i=Math.round(o*100);let r='<div class="rating-container-not-cursore" data-rating="0">';for(let n=1;n<=s;n+=1)r+=`<span class="star-js selected" data-value="${n}">
    ...
    </span>`;return r+"</div>"}function X(t){return t?`<button class="add-favorite-js" type="button" style="font-size: 14px;">
  ...
  </button>`}function Y(t){const e=I(t==null?void 0:t.rating,0),s=Number.isFinite(e)?e.toFixed(1):"—";return`<div class="modal-info" data-id="${u(t==null?void 0:t._id,"")}">
    ...
    <div class="modal-content-wrapper">
      <div class="card-wrapper">
        <h3 class="title-card-modal">${Q(u(t==null?void 0:t.name,""))}</h3>
  ...
  </div>`}function Z(t){return`<div class="modal-get-raiting" data-id="${u(t,"")}">
    <button class="modal-button-close" id="button-close">
      <svg class="close-icon-img" width="20" height="20" aria-label="close-icon">
        <use href="${c}#icon-close"></use>
      </svg>
  ...
  </div>`}function V(){const t=document.querySelector(".rating-container-js"),e=t.querySelectorAll(".exercises_btn_start_icon_rating");let s=t.querySelector(".user-rating-js");e.forEach((a,n)=>{a.addEventListener("mouseover",()=>o(n)),a.addEventListener("mouseout",i),a.addEventListener("click",()=>r(n+1))});function o(a){i();for(let n=0;n<=a;n++)e[n].classList.add("hovered"),s.textContent=`${n+1}`}function i(){e.forEach(a=>{const n=t.dataset.rating;a.classList.remove("hovered"),s.textContent=`${n||0}`})}function r(a){t.setAttribute("data-rating",a),e.forEach((n,L)=>{L+1<=a?n.classList.add("selected"):n.classList.remove("selected")})}}v.settings({timeout:3e3,resetOnHover:!0,icon:"material-icons",transitionIn:"flipInX",transitionOut:"flipOutX",position:"topRight"});class W{error(e){v.error({title:"Error",message:e})}success(e){v.success({title:"OK",message:e})}info(e){v.info({title:"Info",message:e})}}const _=new W,tt=document.querySelector(".filter-list-js");let $,S,T;async function et(t){t.preventDefault();const e=document.querySelector(".modal-get-raiting").dataset.id,o=document.querySelector(".rating-container-js").dataset.rating,i=document.querySelector(".raiting-form-field-input").value,r=document.querySelector(".raiting-form-field-comment").value,a={rate:Number(o),email:i,review:r};d.open();const n=await w.addExercisesRate(e,a);d.close(),n.message?_.error(`${n.message}`):(_.success(`Thank you for your mark - ${a.rate} for ${n.name}`),b.close())}async function st(){const t=document.querySelector(".modal-info").dataset.id;b.close(),b.open(Z(t)),V(),T=document.querySelector(".raiting-form"),T.addEventListener("submit",et)}async function ot(t){const e=document.querySelector(".refresh-button-js"),s=document.querySelector(".modal-info").dataset.id;let i=window.location.href.toString().slice(-14);if(e.dataset.favorite==="false"){e.innerHTML=`<button class="add-favorite-js" type="button">
                                        <span class="remote-favorites">Remove from favorites</span>
                                        <svg class="trash-icon-img" width="18" height="18" aria-label="trash-icon">
                                            <use href="${c}#icon-trash"></use>
                                        </svg>
                                    </button>`,e.dataset.favorite="true",d.open();let r=await w.getOneExercises(s);d.close(),A(r),i==="favorites.html"&&y()}else e.innerHTML=`<button class="add-favorite-js" type="button">
                                        <span>Add to favorites</span>
                                        <svg class="heart-icon-img" width="20" height="20" aria-label="heart-icon">
                                            <use href="${c}#icon-heart"></use>
                                        </svg>
                                    </button>`,e.dataset.favorite="false",M(s),i==="favorites.html"&&y()}function it(t){var i,r,a;const e=t.target;if(e.classList.contains("favourites_btn_trash_icon")){const n=e.dataset.id||((r=(i=e.closest("[data-id]"))==null?void 0:i.dataset)==null?void 0:r.id);if(!n)return;M(n),y();return}const s=e.closest("[data-id]"),o=(a=s==null?void 0:s.dataset)==null?void 0:a.id;o&&(e.classList.contains("favourites_btn_workout")||nt(o))}tt.addEventListener("click",it);const nt=async t=>{if(!t)return;let e;try{d.open(),e=await w.getOneExercises(t)}catch(o){_.error((o==null?void 0:o.message)||String(o));return}finally{d.close()}if(!e){_.error("Exercise not found or request failed");return}let s=!1;try{const o=localStorage.getItem("favorites"),i=o?JSON.parse(o):[];s=Array.isArray(i)&&i.some(r=>(r==null?void 0:r._id)===t)}catch{s=!1}e.favorite=s,b.open(Y(e)),S=document.querySelector(".refresh-button-js"),$=document.querySelector(".add-rating"),$&&$.addEventListener("click",st),S&&S.addEventListener("click",ot)};export{h as a,Q as c,w as f,d as l,_ as m};
//# sourceMappingURL=exercises-right-part-filter-23746523.js.map
